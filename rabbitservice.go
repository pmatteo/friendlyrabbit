...
func NewRabbitService(
	...
	connectionPool, err := NewConnectionPool(config.PoolConfig, nil, nil)
	if err != nil {
		return nil, err
	}
...
func NewRabbitServiceWithPublisher(
	...
	if config.EncryptionConfig.Enabled && len(passphrase) > 0 && len(salt) > 0 {
		rs.Config.EncryptionConfig.Hashkey = crypto.GetHashWithArgon(
			passphrase,
			salt,
...
func (rs *RabbitService) PublishWithConfirmation(
	...
	return rs.Publisher.PublishWithConfirmationTransient(l, time.Duration(time.Millisecond*300))
}
